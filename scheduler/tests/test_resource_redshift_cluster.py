...
def test_scheduler_skips_redshift_cluster_start_on_invalid_cluster_state(lambda_context, redshift_cluster_start):
    with redshift_stubber as stubbed:
        stubbed.add_client_error('resume_cluster', service_error_code='InvalidClusterStateFault')
        response = handler(redshift_cluster_start, lambda_context)
        assert response == {
            'success': False,
...
def test_scheduler_skips_redshift_cluster_stop_on_invalid_cluster_state(lambda_context, redshift_cluster_stop):
    with redshift_stubber as stubbed:
        stubbed.add_client_error('pause_cluster', service_error_code='InvalidClusterStateFault')
        response = handler(redshift_cluster_stop, lambda_context)
        assert response == {
            'success': False,
...
def test_scheduler_redshift_cluster_start_on_non_existing_cluster_raises_error(lambda_context, redshift_cluster_start):
    with pytest.raises(ClientError):
        with redshift_stubber as stubbed:
            stubbed.add_client_error('resume_cluster', service_error_code='ClusterNotFoundFault')
...
def test_scheduler_redshift_cluster_stop_on_non_existing_cluster_raises_error(lambda_context, redshift_cluster_stop):
    with pytest.raises(ClientError):
        with redshift_stubber as stubbed:
            stubbed.add_client_error('pause_cluster', service_error_code='ClusterNotFoundFault')
