...
def test_scheduler_skips_rds_instance_start_on_invalid_instance_state(lambda_context, rds_instance_start):
    with rds_stubber as stubbed:
        stubbed.add_client_error('start_db_instance', service_error_code='InvalidDBInstanceStateFault')
        response = handler(rds_instance_start, lambda_context)
        assert response == {
            'success': False,
...
def test_scheduler_skips_rds_instance_stop_on_invalid_instance_state(lambda_context, rds_instance_stop):
    with rds_stubber as stubbed:
        stubbed.add_client_error('stop_db_instance', service_error_code='InvalidDBInstanceStateFault')
        response = handler(rds_instance_stop, lambda_context)
        assert response == {
            'success': False,
...
def test_scheduler_rds_instance_start_on_non_existing_instance_raises_error(lambda_context, rds_instance_start):
    with pytest.raises(ClientError):
        with rds_stubber as stubbed:
            stubbed.add_client_error('start_db_instance', service_error_code='DBInstanceNotFoundFault')
...
def test_scheduler_rds_instance_stop_on_non_existing_instance_raises_error(lambda_context, rds_instance_stop):
    with pytest.raises(ClientError):
        with rds_stubber as stubbed:
            stubbed.add_client_error('stop_db_instance', service_error_code='DBInstanceNotFoundFault')
