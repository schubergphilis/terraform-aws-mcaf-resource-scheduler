...
class EcsServiceController(ResourceController):
    ...
    def start(self) -> Tuple[bool, str]:
        self.client.update_service(cluster=self.cluster_name, service=self.name, desiredCount=self.desired)
        return (True, f'Service {self.name} started successfully')

    def stop(self) -> Tuple[bool, str]:
        self.client.update_service(cluster=self.cluster_name, service=self.name, desiredCount=0)
