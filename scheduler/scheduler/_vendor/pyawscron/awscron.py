...
class AWSCron:
    ...
    def get_next_n_schedule(n, from_date, cron):
        ...
        if not isinstance(from_date, datetime.datetime):
            raise ValueError(
                "Invalid from_date. Must be of type datetime.dateime"
    ...
    def get_all_schedule_bw_dates(from_date, to_date, cron, exclude_ends=False):
        ...
        if type(from_date) != type(to_date) and not (
            ...
            raise ValueError(
                "The from_date and to_date must be same type." "  {0} != {1}".format(
                    type(from_date), type(to_date)
                )
            )
            ...
            schedule_list = []
            cron_iterator = AWSCron(cron)
            start = from_date.replace(second=0, microsecond=0) - datetime.timedelta(
                seconds=1
            )
